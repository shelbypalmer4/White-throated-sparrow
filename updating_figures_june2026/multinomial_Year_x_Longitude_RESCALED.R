...
##scaling year and longitude (storing means and SDs for later use)
year_scaled <- scale(quant_data$Year)
year_mean <- attr(year_scaled, "scaled:center")
year_sd <- attr(year_scaled, "scaled:scale")
longitude_scaled <- scale(quant_data$Longitude)
longitude_mean <- attr(longitude_scaled, "scaled:center")
longitude_sd <- attr(longitude_scaled, "scaled:scale")
##actually scaling the variables on the dataframe we're using for the regression
quant_data$Year <- c(scale(quant_data$Year))
quant_data$Longitude <- c(scale(quant_data$Longitude))
multinom.m1 <- multinom(poetry ~ Year*Longitude, data = quant_data)
...
#formula for scaling: (x - mean(x)) / sd(x)
#i.e. nd$Year <- (((nd$Year) - year_mean) / year_sd)
years_for_preds_unscaled <- seq(from = 1951, to = 2021, length.out = 71)
years_for_preds <- ((years_for_preds_unscaled - year_mean) / year_sd)

west_longitude_for_pred <- rep(-122.76, 71)
west_longitude_for_pred <- ((west_longitude_for_pred - longitude_mean) / longitude_sd)
central_longitude_for_pred <- rep(-93.25, 71)
central_longitude_for_pred <- ((central_longitude_for_pred - longitude_mean) / longitude_sd)
east_longitude_for_pred <- rep(-73.9, 71)
east_longitude_for_pred <- ((east_longitude_for_pred - longitude_mean) / longitude_sd)
...
multinom.central <- predict(multinom.m1, type = "probs", newdata = nd2)

nd3 <- data.frame(years_for_preds, east_longitude_for_pred)
colnames(nd3) <- c("Year", "Longitude")
multinom.east <- predict(multinom.m1, type = "probs", newdata = nd3)
...
#year_mean: 2002.846
#year_sd: 24.30575
#scaled 1960 = -1.762778
#scaled 1980 = -0.939928
#scaled 2000 = -0.1170776
#scaled 2020 = 0.7057728
#((year - year_mean) / year_sd)
#scale_x_continuous(breaks=seq(-1.762778,-0.939928,-0.1170776,0.7057728),labels=seq(1960,1980,2000,2020))

##Try using cowplot. dashed line is linetype = 3 in stat_function
##unscaled year range: 1951, 2021
song_rhythm_probability_by_year_west <- ggplot(data.frame(x = c(-2.13,0.75)), aes(x)) +
  ylim(0,1) +
  ylab("") +
  xlab("Year") +
  stat_function(fun = cretic_west, colour = "#41B6C4", linewidth = 1.5) +
  stat_function(fun = cretic_lower_west, colour = "#41B6C4", linewidth = 1, linetype = 5) +
  stat_function(fun = cretic_upper_west, colour = "#41B6C4", linewidth = 1, linetype = 5) +
  stat_function(fun = trochaic_west, colour = "#FE9929", linewidth = 1.5) +
  stat_function(fun = trochaic_lower_west, colour = "#FE9929", linewidth = 1, linetype = 5) +
  stat_function(fun = trochaic_upper_west, colour = "#FE9929", linewidth = 1, linetype = 5) +
  stat_function(fun = dactyl_west, colour = "#AE017E", linewidth = 1.5) +
  stat_function(fun = dactyl_lower_west, colour = "#AE017E", linewidth = 1, linetype = 5) +
  stat_function(fun = dactyl_upper_west, colour = "#AE017E", linewidth = 1, linetype = 5) +
  theme_cowplot(25) +
  scale_x_continuous(breaks=seq(-1.762778,0.7057728,0.8228502),labels=c(1960,1980,2000,2020))
ggsave(filename = "rhythms_by_year_west_rescaled.png", plot = song_rhythm_probability_by_year_west)
...
##Try using cowplot. dashed line is linetype = 3 in stat_function
song_rhythm_probability_by_year_central <- ggplot(data.frame(x = c(-2.13,0.75)), aes(x)) +
  ylim(0,1) +
  ylab("") +
  xlab("Year") +
  ...
  theme_cowplot(25) +
  scale_x_continuous(breaks=seq(-1.762778,0.7057728,0.8228502),labels=c(1960,1980,2000,2020))
ggsave(filename = "rhythms_by_year_central_rescaled.png", plot = song_rhythm_probability_by_year_central)
...
##Try using cowplot. dashed line is linetype = 3 in stat_function
song_rhythm_probability_by_year_east <- ggplot(data.frame(x = c(-2.13,0.75)), aes(x)) +
  ylim(0,1) +
  ylab("") +
  xlab("Year") +
  ...
  theme_cowplot(25) +
  scale_x_continuous(breaks=seq(-1.762778,0.7057728,0.8228502),labels=c(1960,1980,2000,2020))
ggsave(filename = "rhythms_by_year_east_rescaled.png", plot = song_rhythm_probability_by_year_east)
